...
module('Integration | Modifiers | modifiers/track-on-deletion', function (hooks) {
  ...
  test('it logs the activity when the element is removed from the DOM', async function (assert) {
    ...
    await render(hbs`
    {{#if this.visible}}
      <div id="destroy-me" {{track-on-deletion "description to log"}}></div>
    {{else}}
  ...
  test('it throws an error if the actionDescription is not passed', async function (assert) {
    ...
    setupOnerror((err: Error) => {
      assert.equal(
        err.message,
        'Assertion Failed: [modifier][track-on-deletion] An actionDescription needs to be passed for the activity-log to make sense.'
      );
    });
    await render(hbs`
    {{#if this.visible}}
      <div id="destroy-me" {{track-on-deletion}}></div>
    {{else}}
      <div id="target-helper"></div>
    {{/if}}
    `);
